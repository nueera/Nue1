...
interface ProductListProps {
  data: BillingProduct[];
  isLoading?: boolean;
  onRowClick?: (item: BillingProduct) => void;
  onEdit?: (item: BillingProduct) => void;
  onDelete?: (item: BillingProduct) => void;
  onCreateNew?: () => void;
}
...
export function ProductList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: ProductListProps) {
  ...
  const columns = useMemo<ColumnDef<BillingProduct>[]>(() => [
    { accessorKey: 'name', header: 'Product' },
    { accessorKey: 'sku', header: 'SKU' },
    { accessorKey: 'productType', header: 'Type', cell: ({ getValue }) => <Badge variant="secondary">{(getValue() as string).replace(/_/g, ' ')}</Badge> },
