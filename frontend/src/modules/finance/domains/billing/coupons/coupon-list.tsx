...
export function CouponList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: CouponListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<Coupon>[]>(() => [
    { accessorKey: 'code', header: 'Code', cell: ({ getValue }) => <code className="px-2 py-0.5 rounded bg-muted text-xs font-mono">{getValue() as string}</code> },
    { accessorKey: 'discountType', header: 'Type', cell: ({ getValue }) => <Badge variant="secondary">{getValue() as string}</Badge> },
    { accessorKey: 'discountValue', header: 'Value', cell: ({ getValue, row }) => `${getValue() as number}${row.original.discountType === 'percentage' ? '%' : ''}` },
    { accessorKey: 'usedCount', header: 'Used' },
    { accessorKey: 'validTo', header: 'Expires' },
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => <Badge variant={getValue() === 'active' ? 'default' : getValue() === 'expired' ? 'destructive' : 'secondary'}>{getValue() as string}</Badge> },
    {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-card flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search coupons..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && <Button onClick={onCreateNew}><Plus className="h-4 w-4 mr-1" />New Coupon</Button>}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No coupons found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
