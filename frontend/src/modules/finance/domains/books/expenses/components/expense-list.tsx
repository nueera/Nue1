...
import { getExpenseStatusColor, getExpenseCategoryLabel } from '../utils';
...
export function ExpenseList({ data, isLoading, onRowClick, onEdit, onDelete, onApprove, onCreateNew }: ExpenseListProps) {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{row.original.vendorName ?? row.original.projectName ?? ''}</p>
        </div>
      ),
    },
    {
      ...
      header: 'Category',
      cell: ({ getValue }) => <span className="text-sm">{getExpenseCategoryLabel(getValue() as Expense['category'])}</span>,
    },
    { accessorKey: 'date', header: 'Date' },
    {
      accessorKey: 'amount',
      header: 'Amount',
      cell: ({ row }) => <MoneyDisplay amount={row.original.amount.amount} currency={row.original.amount.currency} size="sm" />,
    },
    {
      accessorKey: 'status',
      header: 'Status',
      cell: ({ getValue }) => {
        ...
        return <Badge variant="outline" className={cn('text-xs font-medium', getExpenseStatusColor(status))}>{label}</Badge>;
      },
    },
    {
      id: 'actions',
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border bg-glass-bg flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search expenses..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && (
            ...
            <TableBody>
              {isLoading
                ...
                : table.getRowModel().rows?.length
                  ...
                  : (<TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No expenses found</TableCell></TableRow>)}
            </TableBody>
          </Table>
        </div>
      </div>
