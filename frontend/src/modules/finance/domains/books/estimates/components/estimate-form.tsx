    ...
    <div className="glass-surface border border-glass-border rounded-2xl p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <AccessibleField label="Expiry Date" required>
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="grid grid-cols-12 gap-2 mb-2 items-start">
              <div className="col-span-4">
                <input {...register(`lineItems.${index}.item`)} placeholder="Item" aria-label={`Line item ${index + 1} name`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-foreground text-sm outline-none" />
              </div>
              <div className="col-span-3">
                <input {...register(`lineItems.${index}.description`)} placeholder="Description" aria-label={`Line item ${index + 1} description`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-foreground text-sm outline-none" />
              </div>
              <div className="col-span-2">
                <input {...register(`lineItems.${index}.quantity`, { valueAsNumber: true })} type="number" placeholder="Qty" aria-label={`Line item ${index + 1} quantity`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-foreground text-sm outline-none" />
              </div>
              <div className="col-span-2">
                <input {...register(`lineItems.${index}.rate`, { valueAsNumber: true })} type="number" step="0.01" placeholder="Rate" aria-label={`Line item ${index + 1} rate`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-foreground text-sm outline-none" />
              </div>
              <div className="col-span-1 flex items-center justify-center">
                {fields.length > 1 && (
                  <button type="button" onClick={() => remove(index)} className="text-muted-foreground hover:text-destructive" aria-label={`Remove line item ${index + 1}`}><Trash2 className="h-4 w-4" /></button>
                )}
              </div>
            </div>
          ))}
