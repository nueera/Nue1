...
export function EstimateList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew, onConvertToInvoice }: EstimateListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<Estimate>[]>(() => [
    { accessorKey: 'number', header: 'Estimate #' },
    { accessorKey: 'customerName', header: 'Customer' },
    { accessorKey: 'date', header: 'Date' },
    { accessorKey: 'expiryDate', header: 'Expiry' },
    {
      accessorKey: 'total', header: 'Amount',
      cell: ({ row }) => <MoneyDisplay amount={row.original.total.amount} currency={row.original.total.currency} size="sm" />,
    },
    {
      accessorKey: 'status', header: 'Status',
      cell: ({ getValue }) => {
        const status = getValue() as EstimateStatus;
        const info = ESTIMATE_STATUSES.find(s => s.value === status);
        return <Badge variant="outline" className={cn('text-xs font-medium', getEstimateStatusColor(status))}>{info?.label ?? status}</Badge>;
      },
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={e => { e.stopPropagation(); onConvertToInvoice?.(row.original); }}>Convert to Invoice</DropdownMenuItem>
            <DropdownMenuItem onClick={e => { e.stopPropagation(); onDelete?.(row.original); }} className="text-destructive">Delete</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
  ], [onEdit, onDelete, onConvertToInvoice]);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border bg-glass-bg flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search estimates..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && <Button onClick={onCreateNew} className="bg-emerald-600 hover:bg-emerald-700"><Plus className="h-4 w-4 mr-1" />New Estimate</Button>}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No estimates found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
