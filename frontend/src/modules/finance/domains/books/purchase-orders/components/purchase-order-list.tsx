...
export function PurchaseOrderList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: PurchaseOrderListProps) {
  ...
  const columns = useMemo<ColumnDef<PurchaseOrder>[]>(() => [
    ...
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => { const status = getValue() as OrderStatus; const info = PO_STATUSES.find(s => s.value === status); return <Badge variant="outline" className={cn('text-xs font-medium', getPOStatusColor(status))}>{info?.label ?? status}</Badge>; } },
