...
import { getCustomerStatusColor } from '../utils';
...
export function CustomerList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: CustomerListProps) {
  ...
  const columns = useMemo<ColumnDef<Customer>[]>(() => [
    ...
    { accessorKey: 'company', header: 'Company' },
    {
      ...
      cell: ({ getValue }) => {
        ...
        const info = CUSTOMER_STATUSES.find(s => s.value === status);
        return (
          <Badge variant="outline" className={cn('text-xs font-medium', getCustomerStatusColor(status as Customer['status']))}>
            {info?.label ?? status}
          </Badge>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <MoneyDisplay
          amount={row.original.totalRevenue.amount}
          currency={row.original.totalRevenue.currency}
          size="sm"
        />
      ),
    },
              ...
              {isLoading
                ...
                : table.getRowModel().rows?.length
                  ? table.getRowModel().rows.map(row => (
                      <TableRow
                        key={row.id}
                        className={cn('border-b border-glass-border/50 transition-all duration-150 hover:bg-glass-hover', onRowClick && 'cursor-pointer')}
