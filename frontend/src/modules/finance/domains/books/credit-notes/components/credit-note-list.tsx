...
interface CreditNoteListProps { data: CreditNote[]; isLoading?: boolean; onRowClick?: (cn: CreditNote) => void; onEdit?: (cn: CreditNote) => void; onDelete?: (cn: CreditNote) => void; onCreateNew?: () => void; }
...
export function CreditNoteList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: CreditNoteListProps) {
  ...
  const columns = useMemo<ColumnDef<CreditNote>[]>(() => [
    ...
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => { const status = getValue() as CreditNoteStatus; const info = CREDIT_NOTE_STATUSES.find(s => s.value === status); return <Badge variant="outline" className={cn('text-xs font-medium', getCreditNoteStatusColor(status))}>{info?.label ?? status}</Badge>; } },
