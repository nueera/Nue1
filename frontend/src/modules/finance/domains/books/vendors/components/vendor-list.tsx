...
import { getVendorStatusColor } from '../utils';
...
export function VendorList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: VendorListProps) {
  ...
  const columns = useMemo<ColumnDef<Vendor>[]>(() => [
    ...
    { accessorKey: 'company', header: 'Company' },
    {
      ...
      cell: ({ getValue }) => {
        ...
        const info = VENDOR_STATUSES.find(s => s.value === status);
        return (
          <Badge variant="outline" className={cn('text-xs font-medium', getVendorStatusColor(status as Vendor['status']))}>
            {info?.label ?? status}
          </Badge>
        );
      },
    },
    {
      ...
      cell: ({ row }) => (
        <MoneyDisplay
          amount={row.original.outstandingBalance.amount}
          currency={row.original.outstandingBalance.currency}
          size="sm"
          colorize={row.original.outstandingBalance.amount > 0 ? 'negative' : 'neutral'}
        />
      ),
    },
    {
      ...
      header: 'Purchases',
      cell: ({ row }) => (
        <MoneyDisplay amount={row.original.totalPurchases.amount} currency={row.original.totalPurchases.currency} size="sm" />
      ),
    },
    {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border bg-glass-bg flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search vendors..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && (
          <Button onClick={onCreateNew} className="bg-emerald-600 hover:bg-emerald-700"><Plus className="h-4 w-4 mr-1" />New Vendor</Button>
        )}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No vendors found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
