...
import { getSalesOrderStatusColor } from '../utils';
// @ts-expect-error — Module '"../types"' declares 'OrderStatus' locally, but it i...
import type { OrderStatus } from '../types';

interface SalesOrderListProps { data: SalesOrder[]; isLoading?: boolean; onRowClick?: (o: SalesOrder) => void; onEdit?: (o: SalesOrder) => void; onDelete?: (o: SalesOrder) => void; onCreateNew?: () => void; }
...
export function SalesOrderList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: SalesOrderListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<SalesOrder>[]>(() => [
    { accessorKey: 'number', header: 'SO #' },
    ...
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => { const st = getValue() as OrderStatus; const info = SALES_ORDER_STATUSES.find(s => s.value === st); return <Badge variant="outline" className={cn('text-xs font-medium', getSalesOrderStatusColor(st))}>{info?.label ?? st}</Badge>; } },
