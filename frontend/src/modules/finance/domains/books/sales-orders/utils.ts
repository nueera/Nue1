...
export function getSalesOrderStatusLabel(status: OrderStatus): string {
  return ORDER_STATUS_CONFIG[status]?.label ?? status;
}

export function getSalesOrderStatusColor(status: OrderStatus): string {
...
export function isFullyFulfilled(order: SalesOrder): boolean {
  return order.lineItems.every(li => li.fulfilledQuantity >= li.quantity);
}
