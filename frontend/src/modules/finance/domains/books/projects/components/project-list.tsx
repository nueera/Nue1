...
export function ProjectList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: ProjectListProps) {
  ...
  const columns = useMemo<ColumnDef<Project>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <Badge variant="outline" className={cn('text-xs font-medium', getProjectStatusColor(status))}>{label}</Badge>;
      },
    },
    {
      accessorKey: 'budget',
      header: 'Budget',
      cell: ({ row }) => <MoneyDisplay amount={row.original.budget.amount} currency={row.original.budget.currency} size="sm" />,
    },
    {
      accessorKey: 'billed',
      header: 'Billed',
      cell: ({ row }) => <MoneyDisplay amount={row.original.billed.amount} currency={row.original.billed.currency} size="sm" />,
    },
    {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border bg-glass-bg flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search projects..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && (
            ...
            <TableBody>
              {isLoading
                ...
                : table.getRowModel().rows?.length
                  ...
                  : (<TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No projects found</TableCell></TableRow>)}
            </TableBody>
          </Table>
        </div>
      </div>
