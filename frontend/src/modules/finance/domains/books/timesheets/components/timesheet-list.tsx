...
import { ArrowUpDown, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, MoreHorizontal, Plus, Clock } from 'lucide-react';
...
import { getTimeEntryStatusColor, formatHours } from '../utils';
...
export function TimesheetList({ data, isLoading, onRowClick, onEdit, onDelete, onApprove, onCreateNew }: TimesheetListProps) {
  ...
  const columns = useMemo<ColumnDef<TimeEntry>[]>(() => [
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{row.original.projectName ?? 'No project'}</p>
        </div>
      ),
    },
    { accessorKey: 'date', header: 'Date' },
    {
      ...
      cell: ({ getValue }) => (
        <div className="flex items-center gap-1.5">
          ...
          <span className="tabular-nums">{formatHours(getValue() as number)}</span>
        </div>
      ),
    },
    {
      ...
      header: 'Billable',
      cell: ({ row }) => (
        <Badge variant="outline" className={cn('text-xs', row.original.billable ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800' : 'bg-gray-50 text-gray-500 border-gray-200 dark:bg-gray-800/30 dark:text-gray-400 dark:border-gray-700')}>
    ...
    },
    {
      ...
      header: 'Rate',
      cell: ({ row }) => row.original.billable
        ? <MoneyDisplay amount={row.original.billableRate.amount} currency={row.original.billableRate.currency} size="sm" />
    ...
    },
    {
      ...
      header: 'Status',
      cell: ({ getValue }) => {
        const status = getValue() as TimeEntry['status'];
        const label = status.charAt(0).toUpperCase() + status.slice(1);
        return <Badge variant="outline" className={cn('text-xs font-medium', getTimeEntryStatusColor(status))}>{label}</Badge>;
      },
    },
    {
      id: 'actions',
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {row.original.status === 'submitted' && (
              <DropdownMenuItem onClick={e => { e.stopPropagation(); onApprove?.(row.original); }}>Approve</DropdownMenuItem>
            )}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border bg-glass-bg flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search time entries..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && (
            ...
            <TableBody>
              {isLoading
                ...
                : table.getRowModel().rows?.length
                  ...
                  : (<TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No time entries found</TableCell></TableRow>)}
            </TableBody>
          </Table>
        </div>
      </div>
