...
export function PaymentList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: PaymentListProps) {
  ...
  const columns = useMemo<ColumnDef<Payment>[]>(() => [
    ...
    { accessorKey: 'amount', header: 'Amount', cell: ({ row }) => <MoneyDisplay amount={row.original.amount.amount} currency={row.original.amount.currency} size="sm" colorize="positive" /> },
    { accessorKey: 'paymentMethod', header: 'Method', cell: ({ getValue }) => <span className="text-sm">{getPaymentMethodLabel(getValue() as Payment['paymentMethod'])}</span> },
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => <PaymentStatusBadge status={getValue() as PaymentStatus} /> },
