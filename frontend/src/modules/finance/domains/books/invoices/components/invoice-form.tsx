    ...
    <div className="glass-surface border border-glass-border rounded-2xl p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <AccessibleField label="Due Date" required>
            <input {...register('dueDate')} type="date" className="w-full px-3 py-2 rounded-lg border border-glass-border bg-glass-bg text-foreground text-sm outline-none focus:border-emerald-500/50" />
          </AccessibleField>
        </div>
        <AccessibleField label="Subject">
        ...
        <div>
          <div className="flex items-center justify-between mb-2"><span className="text-xs text-muted-foreground font-medium">Line Items</span><Button type="button" variant="ghost" size="sm" onClick={() => append({ item: '', description: '', quantity: 1, rate: 0 })}><Plus className="h-4 w-4 mr-1" />Add Item</Button></div>
          {fields.map((field, i) => (
            <div key={field.id} className="grid grid-cols-12 gap-2 mb-2">
              <div className="col-span-4"><input {...register(`lineItems.${i}.item`)} placeholder="Item" aria-label={`Line item ${i + 1} name`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-sm outline-none" /></div>
              <div className="col-span-3"><input {...register(`lineItems.${i}.description`)} placeholder="Description" aria-label={`Line item ${i + 1} description`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-sm outline-none" /></div>
              <div className="col-span-2"><input {...register(`lineItems.${i}.quantity`, { valueAsNumber: true })} type="number" placeholder="Qty" aria-label={`Line item ${i + 1} quantity`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-sm outline-none" /></div>
              <div className="col-span-2"><input {...register(`lineItems.${i}.rate`, { valueAsNumber: true })} type="number" step="0.01" placeholder="Rate" aria-label={`Line item ${i + 1} rate`} className="w-full px-2 py-1.5 rounded-md border border-glass-border bg-glass-bg text-sm outline-none" /></div>
              <div className="col-span-1 flex items-center justify-center">{fields.length > 1 && <button type="button" onClick={() => remove(i)} className="text-muted-foreground hover:text-destructive" aria-label={`Remove line item ${i + 1}`}><Trash2 className="h-4 w-4" /></button>}</div>
            </div>
          ))}
          {errors.lineItems && <p className="text-xs text-destructive mt-1" role="alert">{errors.lineItems.message}</p>}
        </div>
