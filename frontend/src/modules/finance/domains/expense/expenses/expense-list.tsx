...
export function ExpenseList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: ExpenseListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<Expense>[]>(() => [
    { accessorKey: 'description', header: 'Description' },
    { accessorKey: 'category', header: 'Category', cell: ({ getValue }) => <Badge variant="secondary">{getValue() as string}</Badge> },
    {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {onCreateNew && <Button onClick={onCreateNew}><Plus className="h-4 w-4 mr-1" />New Expense</Button>}
      </div>
