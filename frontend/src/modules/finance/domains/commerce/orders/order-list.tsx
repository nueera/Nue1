...
import type { Money, OrderStatus } from '../../../types/finance-common';
import type { CommerceOrder } from './types';

interface OrderListProps {
  data: CommerceOrder[];
  isLoading?: boolean;
  onRowClick?: (item: CommerceOrder) => void;
  onEdit?: (item: CommerceOrder) => void;
  onCancel?: (item: CommerceOrder) => void;
}
...
export function OrderList({ data, isLoading, onRowClick, onEdit, onCancel }: OrderListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<CommerceOrder>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <Badge variant={s === 'delivered' ? 'default' : s === 'cancelled' ? 'destructive' : 'secondary'}>{s}</Badge>;
      },
    },
    { accessorKey: 'paymentMethod', header: 'Payment', cell: ({ getValue }) => <Badge variant="secondary">{getValue() as string}</Badge> },
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={e => { e.stopPropagation(); onEdit?.(row.original); }}>View</DropdownMenuItem>
            <DropdownMenuItem onClick={e => { e.stopPropagation(); onCancel?.(row.original); }} className="text-destructive">Cancel</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
