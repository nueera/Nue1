...
import type { PaymentPageStatus } from '../types';

interface PaymentPagesListProps {
  data: PaymentPage[];
  isLoading?: boolean;
  onRowClick?: (item: PaymentPage) => void;
  onEdit?: (item: PaymentPage) => void;
  onDelete?: (item: PaymentPage) => void;
  onCreateNew?: () => void;
}
...
export function PaymentPagesList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: PaymentPagesListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<PaymentPage>[]>(() => [
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-card flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search payment pages..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && (
          <Button onClick={onCreateNew}><Plus className="h-4 w-4 mr-1" />New PaymentPage</Button>
        )}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No payment pages found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
