...
interface TransactionListProps {
  ...
  isLoading?: boolean;
  onRowClick?: (item: CheckoutTransaction) => void;
  onRefund?: (item: CheckoutTransaction) => void;
}
...
export function TransactionList({ data, isLoading, onRowClick, onRefund }: TransactionListProps) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [globalFilter, setGlobalFilter] = useState('');

  const columns = useMemo<ColumnDef<CheckoutTransaction>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <Badge variant={s === 'completed' ? 'default' : s === 'failed' ? 'destructive' : 'secondary'}>{s}</Badge>;
      },
    },
    { accessorKey: 'method', header: 'Method' },
    { accessorKey: 'type', header: 'Type', cell: ({ getValue }) => <Badge variant="secondary">{getValue() as string}</Badge> },
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={e => { e.stopPropagation(); onRefund?.(row.original); }}>Refund</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-card flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search transactions..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No transactions found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
