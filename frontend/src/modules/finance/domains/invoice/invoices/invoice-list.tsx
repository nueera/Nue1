...
import type { Money, InvoiceStatus } from '../../../types/finance-common';
...
export function InvoiceList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: InvoiceListProps) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {onCreateNew && <Button onClick={onCreateNew}><Plus className="h-4 w-4 mr-1" />New Invoice</Button>}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No invoices found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
