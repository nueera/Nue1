...
interface EmployeeListProps {
  ...
  isLoading?: boolean;
  onRowClick?: (item: PayrollEmployee) => void;
  onEdit?: (item: PayrollEmployee) => void;
  onDelete?: (item: PayrollEmployee) => void;
  onCreateNew?: () => void;
}
...
export function EmployeeList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: EmployeeListProps) {
  ...
  const columns = useMemo<ColumnDef<PayrollEmployee>[]>(() => [
    ...
    { accessorKey: 'department', header: 'Department', cell: ({ getValue }) => <Badge variant="secondary">{getValue() as string}</Badge> },
    { accessorKey: 'designation', header: 'Designation' },
    {
      accessorKey: 'salary', header: 'Salary',
      cell: ({ getValue }) => { const m = getValue() as Money; return `${m.currency} ${m.amount.toLocaleString()}`; },
    },
    { accessorKey: 'status', header: 'Status', cell: ({ getValue }) => <Badge variant={getValue() === 'active' ? 'default' : 'secondary'}>{(getValue() as string).replace(/_/g, ' ')}</Badge> },
    {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-card flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search employees..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && <Button onClick={onCreateNew}><Plus className="h-4 w-4 mr-1" />Add Employee</Button>}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No employees found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
