...
interface PayRunListProps {
  data: PayRun[];
  isLoading?: boolean;
  onRowClick?: (item: PayRun) => void;
  onProcess?: (item: PayRun) => void;
  onCreateNew?: () => void;
}
...
export function PayRunList({ data, isLoading, onRowClick, onProcess, onCreateNew }: PayRunListProps) {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={e => { e.stopPropagation(); onProcess?.(row.original); }}>Process</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    },
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-card flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search pay runs..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && <Button onClick={onCreateNew}><Plus className="h-4 w-4 mr-1" />New Pay Run</Button>}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No pay runs found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
