...
function SidebarContent({ collapsed, onNavigate }: { collapsed: boolean; onNavigate?: () => void }) {
  ...
  const { activeProduct, setSidebarActiveItem, toggleSidebar, isRecentlyUsed } = useFinanceStore();
  ...
  const pathAfterFinance = pathname?.split('/finance/')[1] || `${activeProduct}/dashboard`;
  const allSlugs = allItems.map((i) => i.slug);
  const matchedSlug = allSlugs.find((slug) => pathAfterFinance === slug || pathAfterFinance.startsWith(slug + '/')) || `${activeProduct}/dashboard`;
          ...
          collapsed ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center justify-center w-9 h-9 rounded-full bg-module-finance/15 text-module-finance font-semibold text-xs hover:ring-1 hover:ring-module-finance/20 transition-all duration-[var(--motion-fast)] press-scale" suppressHydrationWarning>
                  {user.avatar}
                </button>
              </DropdownMenuTrigger>
            ...
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                <AvatarFallback className="bg-module-finance/15 text-module-finance text-xs font-semibold">
                  {user.avatar}
                </AvatarFallback>
              </Avatar>
              <motion.div
...
export function FinanceSidebar() {
  const { sidebarCollapsed, mobileSidebarOpen, setMobileSidebarOpen } = useFinanceStore();
  const hydrated = useStoreHydrated();
