...
import { useCrmUIStore, useCrmAuthStore } from '../../core/store';
        ...
        <AnimatePresence mode="wait">
          {!collapsed && (
            ...
            >
              ...
              >
                <span className="text-module-crm">Nue</span>CRM
              </span>
            </motion.div>
          )}
          {collapsed && (
            <motion.div
              ...
              className="flex items-center justify-center w-8 h-8 rounded-lg bg-module-crm/15 text-module-crm"
            >
              <LayoutDashboard className="h-4 w-4" strokeWidth={1.8} />
            </motion.div>
          )}
          ...
          collapsed ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center justify-center w-9 h-9 rounded-full bg-module-crm/15 text-module-crm font-semibold text-xs hover:ring-1 hover:ring-module-crm/20 transition-all duration-[var(--motion-fast)] press-scale" suppressHydrationWarning>
                  {user.avatar}
                </button>
              </DropdownMenuTrigger>
            ...
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                <AvatarFallback className="bg-module-crm/15 text-module-crm text-xs font-semibold">
                  {user.avatar}
                </AvatarFallback>
              </Avatar>
              <motion.div
...
export function CrmSidebar() {
  const { sidebarCollapsed, mobileSidebarOpen, setMobileSidebarOpen } = useCrmUIStore();
  const hydrated = useStoreHydrated();
    ...
    return (
      <Sheet open={mobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
        <SheetContent side="left" className="w-72 p-0 surface-sidebar glass-surface-strong border-r border-glass-border">
          ...
          <CrmSidebarContent collapsed={false} onNavigate={() => setMobileSidebarOpen(false)} />
        </SheetContent>
      </Sheet>
    );
  }
