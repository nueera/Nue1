...
export function LeadList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: LeadListProps) {
  ...
  const columns = useMemo<ColumnDef<Lead>[]>(() => [
    ...
    { accessorKey: 'company', header: 'Company' },
    {
      ...
      cell: ({ getValue }) => {
        ...
        const info = LEAD_STATUSES.find(s => s.value === status);
        return <span className={cn('text-xs font-medium px-2 py-1 rounded-full', info?.color, `bg-${info?.color.replace('text-', '')}/15`)}>{info?.label ?? status}</span>;
      },
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        const info = LEAD_RATINGS.find(r => r.value === rating);
        return <span className={cn('text-xs font-medium', info?.color)}>{info?.label ?? rating}</span>;
      },
    },
    {
      accessorKey: 'leadScore', header: 'Score',
      cell: ({ getValue }) => {
        const score = getValue() as number;
        return <span className={cn('text-sm font-semibold', getLeadScoreColor(score))}>{score}</span>;
      },
    },
    { accessorKey: 'leadSource', header: 'Source' },
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border bg-glass-bg flex-1 max-w-md">
          <input type="text" value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} placeholder="Search leads..." className="flex-1 bg-transparent text-foreground placeholder:text-muted-foreground/60 outline-none text-sm" />
        </div>
        {onCreateNew && (
          <Button onClick={onCreateNew} className="bg-module-crm hover:bg-module-crm/80"><Plus className="h-4 w-4 mr-1" />New Lead</Button>
        )}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <TableRow><TableCell colSpan={columns.length} className="h-48 text-center text-muted-foreground">No leads found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
