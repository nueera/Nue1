...
export function AccountList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: AccountListProps) {
  ...
  const columns = useMemo<ColumnDef<Account>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <span className="text-xs font-medium text-muted-foreground">{info?.label ?? type}</span>;
      },
    },
    {
      accessorKey: 'tier',
      ...
      cell: ({ getValue }) => {
        ...
        const info = ACCOUNT_TIERS.find(t => t.value === tier);
        return <span className={`text-xs font-medium ${info?.color}`}>{info?.label ?? tier}</span>;
      },
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <span className="text-xs text-muted-foreground">{formatIndustry(industry as any)}</span>;
      },
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <span className="text-sm text-foreground">${(revenue / 1000000).toFixed(1)}M</span>;
      },
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <span className={`text-sm font-medium ${getHealthScoreColor(score)}`}>{score}</span>;
      },
    },
    {
      id: 'actions',
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-glass-border glass-surface flex-1 max-w-md">
          <input
            type="text"
            value={globalFilter}
        ...
        {onCreateNew && (
          <Button onClick={onCreateNew} className="bg-module-crm hover:bg-module-crm/80">
              ...
              {isLoading ? (
                ...
                table.getRowModel().rows.map(row => (
                  <TableRow
                    key={row.id}
                    className={cn(
                      'border-b border-glass-border/50 transition-all duration-150 hover:bg-glass-hover',
