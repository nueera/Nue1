...
export function ContactList({ data, isLoading, onRowClick, onEdit, onDelete, onCreateNew }: ContactListProps) {
  ...
  const columns = useMemo<ColumnDef<Contact>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        const info = CONTACT_STATUSES.find(s => s.value === status);
        return (
          <span className={cn('text-xs font-medium px-2 py-1 rounded-full', getContactStatusColor(status))}>
            {info?.label ?? status}
          </span>
        );
      },
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <span className="text-xs font-medium text-muted-foreground">{getContactTypeLabel(type)}</span>;
      },
    },
    {
      accessorKey: 'phone',
