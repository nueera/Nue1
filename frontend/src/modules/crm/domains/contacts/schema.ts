...
export const contactMergeSchema = z.object({ primaryId: z.string().min(1), secondaryIds: z.array(z.string()).min(1), fieldResolutions: z.record(z.string(), z.enum(["primary","secondary"])).optional() });
export const contactMassUpdateSchema = z.object({ recordIds: z.array(z.string()).min(1), updates: z.record(z.string(), z.unknown()) });
